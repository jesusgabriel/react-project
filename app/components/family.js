...
import {children, numberVotes} from '../top.js';
        ...
        <ul>
          <li>
            ...
            <div className="countV">
              <button className="btnVotes" onClick={this.handleNumbVotes}>Points</button>
              {this.props.votes}
            </div>
...
class FatherChild extends React.Component {
  ...
  handleChildUpVotes(childIds) {
    ...
    this.setState({child: nextChild});
  }

  render() {
    // console.log(this.state.child);

    const sortedVotes = this.state.child.sort((a, b) => (b.votes - a.votes));

    const displayInfo = this.state.child.map((child) => (<Child key={'child-' + child.id} firstName={child.firstName} lastName={child.lastName} gender={child.gender} img={child.img} onVote={this.handleChildUpVotes} votes={child.votes}/>));
